# server-ssh-tools

Этот скрипт автоматически настраивает **SSH-сервер** максимально безопасным образом:  
- применяет рекомендуемые параметры безопасности в `sshd_config`;  
- добавляет публичные ключи администратора с GitHub;  
- устанавливает кастомный баннер (`/etc/issue.net`);  
- перезапускает SSH **только при изменениях** (идемпотентность).  

---

## ✨ Возможности
- 📌 Поддержка **root-входа только по ключам** (без паролей).  
- 📌 Автоматическая загрузка ключей с GitHub (`https://github.com/<username>.keys`), с проверкой формата перед добавлением.  
- 📌 Идемпотентность — повторный запуск ничего не меняет, если всё уже настроено.  
- 📌 Резервная копия оригинального `sshd_config` создаётся один раз, плюс копия перед каждым запуском для автоматического отката.  
- 📌 Проверка синтаксиса конфига (`sshd -t`) перед перезапуском SSH — при ошибке конфиг откатывается автоматически.  
- 📌 Скрипт требует запуска от root, проверяет наличие нужных команд (`curl`, `sshd`, `systemctl`, `flock`) и не даёт запустить два экземпляра одновременно.  
- 📌 Совместимость со старыми и новыми версиями OpenSSH (`KbdInteractiveAuthentication` добавляется только начиная с OpenSSH 8.7).  
- 📌 Поддержка SELinux (RHEL/CentOS) — контексты `authorized_keys` восстанавливаются автоматически.  
- 📌 Права и владелец `sshd_config` приводятся к `600 root:root`.  
- 📌 Красивый баннер при входе.  

---

## ⚙️ Установка и использование

### 0. Быстрый запуск одной командой
```bash
curl -s https://raw.githubusercontent.com/whereisasan/server-ssh-tools/refs/heads/main/secure_ssh.sh | bash
```
### 1. Склонировать репозиторий
```bash
git clone https://github.com/whereisasan/server-ssh-tools.git
cd server-ssh-tools
```
### 2. Сделать скрипт исполняемым
```bash
chmod +x secure_ssh.sh
```
### 3. Запустить
```bash
./secure_ssh.sh
```